...
export default {
  ...
  config: {
      showSuccessMessage: {
          ...
          description: 'Show success message after saving descriptor',
          type: 'boolean',
          default: true,
          order: 1
      },
      showAdditionalInformation: {
          ...
          description: 'Show saved description path',
          type: 'boolean',
          default: false,
          order: 2
      }
  ...
  async saveDescriptor(filePath, fileContent) {
      ...
      if (atom.config.get('update-descriptor.showAdditionalInformation')) {
          config.detail = "Descriptor by path " + filePath + " is updated";
      }

      if (atom.config.get('update-descriptor.showSuccessMessage')) {
          atom.notifications.addSuccess("Descriptor file is updated", config);
      }
  },
